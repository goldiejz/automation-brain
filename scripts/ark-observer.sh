...
for p in patterns:
    if re.search(p["regex"], line, re.IGNORECASE):
        ...
        print(f"📍 {p['id']} [{p['severity']}] from {source}", file=sys.stderr)
        break
PYEOF
}
...
for pattern_id, count in counts.items():
    ...
    if count >= threshold and pattern_id not in already_lessoned:
        # Get recent occurrences of this pattern
        recent = [o for o in observations if o["pattern_id"] == pattern_id][-5:]

        lesson_path = os.path.join(lessons_dir, f"{pattern_id}.md")
        with open(lesson_path, "w") as f:
            f.write(f"# Observer-Derived Lesson: {pattern_id}\n\n")
            f.write(f"**Source:** ark-observer (continuous monitoring)\n")
            f.write(f"**Captured:** {datetime.datetime.utcnow().isoformat()}Z\n")
            f.write(f"**Category:** {p.get('category', '?')}\n")
            f.write(f"**Severity:** {p.get('severity', '?')}\n")
            f.write(f"**Occurrences:** {count} (threshold: {threshold})\n\n")
            f.write(f"## Pattern\n\n")
            f.write(f"\`{p.get('regex', '')}\`\n\n")
            f.write(f"## Description\n\n")
            f.write(f"{p.get('description', 'No description')}\n\n")
            f.write(f"## Recent Occurrences\n\n")
            for o in recent:
                f.write(f"- **{o['timestamp']}** in \`{o['source']}\`:\n")
                f.write(f"  \`\`\`\n  {o['line']}\n  \`\`\`\n")
            f.write(f"\n## Auto-Fix Suggestion\n\n")
            f.write(f"`{p.get('auto_fix', 'none')}`\n")
            f.write(f"\n*Generated by ark-observer — review and codify into ark scripts if confirmed.*\n")

        print(f"📝 Lesson emitted: {pattern_id} ({count}x)")
        new_lessons += 1
